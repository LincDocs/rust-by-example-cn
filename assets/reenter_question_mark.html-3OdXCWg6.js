import{_ as s,c as n,f as a,o as r}from"./app-w3dN5Ldv.js";const t={};function p(i,e){return r(),n("div",null,e[0]||(e[0]=[a(`<h1 id="的其他用法" tabindex="-1"><code>?</code> 的其他用法</h1><p>注意在上一个例子中，我们调用 <code>parse</code> 后总是立即将错误从标准库的错误 <code>map</code>（映射）到装箱错误。</p><div class="language-rust,ignore line-numbers-mode" data-highlighter="shiki" data-ext="rust,ignore" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-rust,ignore"><span class="line"><span>.and_then(|s| s.parse::&lt;i32&gt;()</span></span>
<span class="line"><span>    .map_err(|e| e.into())</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><p>因为这个操作很简单常见，如果有省略写法就好了。遗憾的是 <code>and_then</code> 不够灵活，所以实现不了这样的写法。不过，我们可以使用 <code>?</code> 来代替它。</p><p><code>?</code> 之前被解释为要么 <code>unwrap</code>，要么 <code>return Err(err)</code>，这只是在大多数情况下是正确的。<code>?</code> 实际上是指 <code>unwrap</code> 或 <code>return Err(From::from(err))</code>。由于 <code>From::from</code> 是不同类型之间的转换工具，也就是说，如果在错误可转换成返回类型地方使用 <code>?</code>，它将自动转换成返回类型。</p><p>我们在这里使用 <code>?</code> 重写之前的例子。重写后，只要为我们的错误类型实现 <code>From::from</code>，就可以不再使用 <code>map_err</code>。</p><div class="language-rust,editable line-numbers-mode" data-highlighter="shiki" data-ext="rust,editable" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-rust,editable"><span class="line"><span>use std::error;</span></span>
